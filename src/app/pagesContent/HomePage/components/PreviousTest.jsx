import React from "react";

const PreviousTest = ({ imagesHeight }) => {
  return (
    <div className="test-section min-w-[320px] max-w-xs bg-[#272727] p-4 pl-0">
      <div className="flex items-center">
        <div className="bg-[#1E1E1E] rounded-full w-8 h-8 flex justify-center items-center">
          <img width="16px" src="/icons/previous-test.svg" alt="" />
        </div>
        <p className="ml-2 text-white text-base">Previous tests</p>
        <div className="ml-auto bg-[#1D1D1D] rounded-md w-8 h-8 flex justify-center items-center cursor-pointer">
          <img width="14px" src="/icons/sort.svg" alt="" />
        </div>
      </div>
      <div
        style={{ maxHeight: imagesHeight?imagesHeight + 45 + "px":"71vh" }}
        className={`overflow-y-auto pr-2 mt-3`}
      >
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
        <div className="flex justify-between items-center rounded-md bg-[#181818] test-gradient p-2 mb-3 cursor-pointer text-[#7E7E7E] hover:text-white">
          <img width="56px" src="/images/test-image.png" alt="" />
          <p className=" mx-2 w-[calc(100%-80px)] text-sm">
            Lorem ipsum dolor sit amet consectetur.
          </p>
          <img width="24px" src="/icons/dots-horizontal.svg" alt="" />
        </div>
      </div>
    </div>
  );
};

export default PreviousTest;
